...
for task in Tasks:
    ...
    #                  'PVh=0.0087872.txt','PVh=0.00442309.txt','PVh=0.00221383.txt']

    #h = [0.12803687993289598, 0.06772854614785964, 0.03450327796711771, 0.017476749542968805,\
    #    0.008787156237382746,0.00442309,0.00221383]
    ...
    #VoronoiIntErr      = [0]*len(ProcessedFiles)
    i                   = 0
    for Pfile in ProcessedFiles:
        ...
        if task == 'MFD':
            Bh,Eh,Berror,Eerror = MFDSolver(J,Nodes,EdgeNodes,ElementEdges,BoundaryNodes,Orientations,EssentialBoundaryCond,InitialCond,ExactE,ExactB,T,dt,theta)
        if task == 'E':
            Bh,Eh,EhI,Berror,Eerror,IntErr,t = ESolver(J,Nodes,EdgeNodes,ElementEdges,\
                                        BoundaryNodes,Orientations,EssentialBoundaryCond,InitialCond,ExactE,ExactB,T,dt,theta)
        if task == 'LS':
            Bh,Eh,EhI,Berror,Eerror,IntErr,t = LSSolver(J,Nodes,EdgeNodes,ElementEdges,\
            BoundaryNodes,Orientations,EssentialBoundaryCond,InitialCond,\
                ExactE,ExactB,T,dt,theta)
        if task == 'GI':
            Bh,Eh,EhI,Berror,Eerror,IntErr,t = GISolver(J,Nodes,EdgeNodes,ElementEdges,\
            BoundaryNodes,Orientations,EssentialBoundaryCond,InitialCond,\
                ExactE,ExactB,T,dt,theta)
    ...
    #print('Voronoi Int'+str(VoronoiIntErr))
    #print('t='+str(t))
    ...
    #               'PertPQh=0.021739.txt','PertPQh=0.010989.txt','PertPQh=0.00549451.txt','PertQh=0.00275482.txt']

    #h = [0.16666666666666666, 0.08333333333333333, 0.043478260869565216, 0.021739130434782608,\
    #     0.010989010989010988,0.00549451,0.00275482]
    ...
    #QuadIntErr      = [0]*len(ProcessedFiles)
    i = 0
    for Pfile in ProcessedFiles:
        dt = 0.05*h[i]**2
        #dt = h[i]**2
    ...
    #                 'PTh=0.0062613.txt','PTh=0.00313997.txt','PTh=0.00156773.txt']


    #h = [0.10101525445522107, 0.05018856132284956, 0.025141822757713456, 0.012525468249897755,\
    #     0.006261260829309998,0.00313997,0.00156773]
    ...
    #print('Triangle Interior = '+str(TriangleIntErr))
    #print('t='+str(t))
